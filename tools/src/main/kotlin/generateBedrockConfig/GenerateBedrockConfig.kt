...
class GenerateBedrockConfig: Subcommand("generate-bedrock-config", "Convert the Lucky Block config files into bedrock-compatible formats") {
    private val inputConfigFolder by option(ArgType.String, description = "Input config folder").default(".")
    private val inputTemplateAddonFolder by option(ArgType.String, description = "Input template addon folder").default("../bedrock/template-addon")
    ...
    override fun execute() {
        ...
        val behaviorPackFolder = File(inputTemplateAddonFolder).resolve("./behavior_pack")
        val resourcePackFolder = File(inputTemplateAddonFolder).resolve("./resource_pack")
        for (inputFile in behaviorPackFolder.walk() + resourcePackFolder.walk()) {
            if (inputFile.isFile) {
                val relativeInputFilePath = inputFile.relativeTo(File(inputTemplateAddonFolder)).path
